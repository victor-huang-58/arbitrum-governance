...
Calibrating to the data, the forum generated net social benefits equivalent to saving at least 555 words of delegate research effort per proposal---all destroyed at the tipping point.
...
\]
which is positive for $K \geq 19$. Arbitrum's 62 identified active delegates ($K = 62$) imply net social benefits of at least $62 \times 555 - 10{,}480 \approx 24{,}000$ words per proposal---about 2.3 times the human writing effort invested in each thread---entirely from the public-good externality of the forum. Because 454 unique community members posted in pre-period proposal threads (and readership exceeds participation), $K \geq 62$ is itself a lower bound, so the $24{,}000$-word figure compounds two conservative margins.

At the tipping point, the delegate's net reading value falls from at least 555 words to exactly 0 in a single step, and at least $24{,}000$ words of social surplus per proposal is destroyed. After the break, human contributors still write $\approx 8{,}858$ words per proposal (the post-period average), but delegates no longer read them---a pure transfer of human effort with zero social return. The welfare loss in Section~\ref{sec:welfare} is thus not a continuous erosion but a cliff: a realized AI share as low as $5.3\%$ marks the point at which the forum transitions from a public good worth at least $24{,}000$ words per proposal to a $0$-word-per-proposal waste.

\paragraph{Delegate alignment and the direction of the bound.}
The model gives the delegate a $\pm 1$ correctness payoff, making it a perfect agent of the DAO. That is a strong assumption here: a simple majority requires only about four delegates, so almost every delegate is non-pivotal and the private return to a correct vote is near zero (rational ignorance); the delegation market rewards \emph{observable activity}---posts, rationales, participation---rather than decision quality; and professional delegates, grant recipients, and vote renters face conflicts. Misalignment does not weaken the calibration---it sharpens its interpretation in two ways. First, a razor-thin private return to reading ($c_r/I_H \ge 0.95$, from $\pi^* \ge 0.053$) is exactly what a non-pivotal, activity-rewarded delegate market \emph{should} generate: the calibration found the fragility, and misalignment explains why it is there. Second, reading is itself a public good---an informed vote benefits every token holder---so the private $I_H$ that governs the reading decision is smaller than the social value of the same post. The collapse therefore reveals the \emph{private} surplus margin, and the social value destroyed \emph{exceeds} the figures above: ``at least 555 words'' becomes more conservative once delegates are misaligned, not less. A corollary is that the socially optimal reading threshold lies above the private $\pi^*$---delegates disengage too early from society's standpoint even before AI, and the AI shock only makes an already-inefficient margin bind.
...
Pearson $r$ is reported as a descriptive effect size; significance stars apply only to
the OLS coefficients and Chow statistic, which share the heteroskedasticity-robust
inference framework.
...
realized AI posts are approximately 5\% of forum volume; because the delegate's
expected contamination $\pi$ runs ahead of this realized share, the break implies
$\pi^* \ge 0.05$ (Section~\ref{sec:calibration}).
The threshold condition then gives $c_{\text{read}} / I_H = 1 - \pi^* \le 0.95$:
reading costs absorb at most 95\% of the informational value of a perfectly reliable post.
...
The concern is ruled out by timing, exactly as for the vesting cliff. DIP was
introduced during 2024---after the QLR endogenous break (November 25, 2023) and the
onset of the readership decline (November 2023) by many months. Whether one dates
the program to its early pilots or to its formal launch later that year, it
postdates the signal collapse; a subsidy that begins after the signal has already
collapsed cannot be its cause. If anything, DIP works \emph{against} the volume
...
\begin{figure}[H]
  ...
  \caption{\textbf{Independent Evidence for the AI Indistinguishability Threshold.}
    ...
    marks chance (50\%) and the dotted line the human baseline ($\approx$66\%);
