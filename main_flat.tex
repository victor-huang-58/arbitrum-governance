...
%% ── Title ─────────────────────────────────────────────────────────────────────
\title{\textbf{AI and the Market for Governance Lemons}}
...
$r = -0.39$ ($p = 0.009$, $n = 44$ proposals): more posts, more disagreement,
...
($p = 0.66$, $n = 108$) in the post-shock period. The change in the correlation
coefficient, $\Delta r = +0.35$, is large, precisely estimated, and confirmed by a
Chow test that rejects parameter stability at $F = 8.22$ ($p = 0.0004$). We use Claude~3 (March 2024) as our primary break date: the pre-period correlation is $r = -0.39$ ($p = 0.009$, $n = 44$) and the post-period correlation is $r = -0.04$ ($p = 0.66$, $n = 108$), providing a clean before-and-after contrast. All substantive conclusions hold at any candidate break date from November 2023 onward (Table~\ref{tab:chow_battery}).

The identification logic is clean. AI model releases are global events determined
by the research roadmaps of Anthropic, OpenAI, and other technology companies---they
are plainly exogenous to any specific Arbitrum proposal. The release of Claude~3 was
not anticipated by the Arbitrum community, was not timed to coincide with any
particular governance vote, and affected all future proposal threads uniformly. The
shock is therefore a valid instrument for the transition from a low-AI to a high-AI
posting environment, and the before--after comparison is interpretable as causal.
...
falsified by timing. Panel~A of Figure~\ref{fig:main_causal} shows that the actual
surge in AI-generated post volume did not arrive until late 2024, nine to twelve
months after the structural break. At the date of the QLR break (November 2023),
AI posts constituted less than 5\% of forum volume. A volume mechanism requires
contamination to precede signal collapse; the data show the reverse.
...
became indistinguishable from careful human writing. Once this threshold was crossed,
...
\emph{Second}, we provide the first causal identification of the effect of AI-generated
content on collective decision mechanisms. The AI shock literature
...
identification argument. Section~\ref{sec:results} presents the main results.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig01_forum_activity.png}
...
coefficient}---the minimum number of independent actors needed to collude and
control a majority of votes---is as low as 4 at the 51\% threshold and 3 at the
33\% blocking-minority threshold (Figure~\ref{fig:power_concentration}).
A rising Nakamoto coefficient over time indicates power becoming more dispersed and
governance more competitive; a falling one indicates consolidation. Ours is broadly
stable across the sample period, consistent with prior evidence of
plutocratic tendencies in DAO governance \citep{barbereau2023}.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig03_power_concentration.png}
  \caption{\textbf{Evolution of Voting Power Concentration.}
    ...
    top-10 delegates. Concentration is high and broadly stable: a majority can
    be formed by as few as 4--7 delegates throughout the sample period.}
...
half-year period. A notable feature is the concentration of ecosystem program
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig02_proposals.png}
...
call it. All posts were scored in a single batch run in April 2025, so model
version is constant across the corpus.} Pangram analyzes the linguistic
...
We retain 152 proposals with a computable margin over the sample period. We exclude
proposals with non-binary choice structures (e.g., ranked-choice elections for
Security Council seats) that do not map cleanly to a For/Against margin.
...
The matching procedure yields 152 matched proposal--thread pairs. Of the 152 Snapshot
proposals with computable margins, 24 (16\%) remain unmatched, primarily proposals
that were submitted with titles substantially different from their forum discussion
threads (e.g., proposals that evolved significantly during community discussion before
being renamed at the Snapshot stage). Unmatched proposals are excluded from the main
analysis; we verify in Section~\ref{sec:robustness} that their exclusion does not
...
addresses appear in our proposal-vote records, of whom only 54 (0.4\%) are linked
...
post--Claude~3 covers March~4, 2024 and later. The Chow test evaluates the null hypothesis
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{fig08_main_causal.png}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig_ai_post_content.png}
...
became objectively busier.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig15_reads_decline.png}
...
Figure~\ref{fig:power_concentration} shows Nakamoto coefficients were broadly stable through
2023--2024; the structural break in power concentration does not coincide with
the November 2023 forum-signal break.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig11_delegates.png}
...
interpretation of AI adoption in DAO governance. High-power delegates---who already
hold substantial delegated voting power and have strong incentives to maintain
credibility with their delegators---do not disproportionately adopt AI. Instead,
it is lower-tier delegates, for whom the opportunity cost of producing high-quality
governance prose is high relative to their marginal influence, who find AI tools
most attractive as a means of maintaining forum visibility at lower cost.
...
seeking to obscure the degree of community division---a concerning scenario in which
the technology enables a form of information manipulation that advantages incumbent
power. Instead, the evidence suggests a more benign mechanism: AI tools democratize
participation at the margin, but in doing so, they dilute the signal quality of the
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{output/figures/rob03_cooks_bootstrap.png}
...
The mechanism is dilution, not manipulation. AI-generated posts carry zero
information about vote outcomes (their correlation with margins is $-0.013$
throughout the sample), and AI adoption is concentrated among lower-tier delegates
with minimal voting power, not among the high-power insiders who might strategically
use it. The forum's signal degrades because posting costs have collapsed for
marginal participants, flooding threads with content that looks like deliberation
but contains no genuine preference signal.
...
with a public JSON API. We retrieve topic lists by category using the endpoint
\texttt{/c/\{category\_id\}/l/latest.json} with page-based pagination, then
retrieve individual topic posts using \texttt{/t/\{topic\_id\}.json} plus chunked
post-ID requests for topics with more than 20 posts. The scraper respects a
one-second delay between requests to avoid server overload.
...
\begin{figure}[H]
  ...
  \caption{\textbf{Incentives and Participation: DIP Test.}
    Scatter of proposal-level ARB value at stake (log scale, x-axis) against
    participation rate (unique voters / circulating supply, y-axis). The
    positive relationship is consistent with rational participation: higher
    stakes proposals attract more voter attention. Fitted line from OLS
    with 95\% confidence interval. The DIP (Decreasing Incentives to
    Participate) test following \citet{gillan2010} evaluates whether
    participation is increasing in proposal economic significance.}
  \label{fig:dip}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{fig14_distraction.png}
...
\end{tabular}
\begin{flushleft}
\small\textit{Notes:} Each row reports OLS estimates of the effect of the indicated
