...
governance prose.
Interestingly, the signal collapse occurs when relatively few posts are AI-generated---approximately 5\% of forum volume at the time of the structural break, compared to 15--20\% by end of sample as AI posting volume grows rapidly.
We use a model of endogenous posting and voting to show that this may be a general phenomenon: the signal value of the forum can collapse to zero at a discrete tipping point with a very low volume of AI posts.
Calibrating the model to the data, using average post length (237 words) as the unit of writing effort, we estimate that the forum generated net social benefits equivalent to saving approximately 555 words of delegate research effort per proposal, all of which is destroyed at the tipping point.
...
Chow test that rejects parameter stability at $F = 8.38$ ($p = 0.0004$). We use Claude~3 (March 2024) as our primary break date: the pre-period correlation is $r = -0.33$ ($p = 0.08$, $n = 29$) and the post-period correlation is $r = -0.10$ ($p = 0.39$, $n = 73$), which provides a clean before-and-after contrast. All substantive conclusions hold at any candidate break date from November 2023 onward (Table~\ref{tab:chow_battery}).
...
AI posts constituted approximately 5\% of forum volume at the QLR break (November 2023) and approximately 5\% at the Claude~3 break (March 2024). The large-scale volume surge did not arrive until late 2024, reaching 15--20\% of forum traffic nine to twelve months after the signal collapsed. A volume mechanism requires contamination to precede collapse; the data show the reverse.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{fig05_ai_score_bimodality.png}
  \begin{singlespace}
...
To calibrate $\pi^*$, we use the QLR endogenous break date (November 25, 2023), at which realized AI posts constitute 5.3\% of forum volume. If we take this as the point at which the delegate's perceived contamination probability $\pi$ first crossed the reading threshold---consistent with the unverifiability mechanism in which $\pi$ tracks the public availability of capable AI---then $\pi^* = 0.053$. Substituting into equation (\ref{eq:pistar}):
\[
  c_r = I_H (1 - \pi^*) = 0.947 \cdot I_H.
\]
Normalizing $I_H = c_p = 237$ words (the information value of one genuine post equals the cost of writing one), this gives $c_r = 224$ words of reading and interpretation effort per post.

The net reading value per delegate per post in the no-AI equilibrium is:
\[
  I_H - c_r = 237 - 224 = 13 \text{ words per post}.
\]
The forum is thus fragile by design: each post provides only 13 words' worth of net informational surplus per delegate---just 5.3\% of its gross information value---so that a 5.3\% contamination rate is precisely sufficient to push the expected return on reading to zero. Integrating across the thread:
\[
  \bar{n} \cdot (I_H - c_r) = 44.2 \times 13 \approx 555 \text{ words per delegate per proposal}.
\]
If $K$ delegates read the forum, the net social surplus per proposal is:
\[
  K \cdot 555 - \bar{n} \cdot c_p = 555K - 10{,}480 \text{ words},
\]
which is positive for $K \geq 19$. Arbitrum's 62 identified active delegates ($K = 62$) implies net social benefits of approximately $62 \times 555 - 10{,}480 \approx 24{,}000$ words per proposal---about 2.3 times the human writing effort invested in each thread---entirely from the public-good externality of the forum. Because 454 unique community members posted in pre-period proposal threads (and readership exceeds participation), $K \geq 62$ is a lower bound.

At the tipping point, the delegate's net reading value falls from 555 to exactly 0 in a single step, and 24{,}000 words of social surplus per proposal is destroyed. After the break, human contributors still write $\approx 8{,}858$ words per proposal (the post-period average), but delegates no longer read them---a pure transfer of human effort with zero social return. The welfare loss in Section~\ref{sec:welfare} is thus not a continuous erosion but a cliff: the 5.3\% AI contamination threshold marks the exact point at which the forum transitions from a $24{,}000$-word-per-proposal public good to a $0$-word-per-proposal waste.
...
Pearson $r$         & $-0.21$\sym{**}& $-0.33$\sym{*}      & $-0.10$       \\
\midrule
