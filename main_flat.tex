...
\author{
  Victor Huang$^{*}$ \quad Joseph Hall$^{*}$ \\[0.5em]
...
A model of endogenous posting, reading, and voting---derived from primitives---shows this is a general phenomenon: the signal value of a forum collapses at a discrete tipping point at a realized A.I.\ share strictly below the level that would mechanically destroy reading value, and the collapse is hysteretic: reducing A.I.\ volume afterward does not restore the forum.
Calibrating to the data, the forum generated net social benefits equivalent to saving at least 858 words of research effort per delegate per proposal (under a stated non-redundancy assumption)---all destroyed at the tipping point.
Delegates who rely on AI hold far less voting power than those who do not, which is inconsistent with power capture by insiders.
Our results suggest that the mere availability of indistinguishable AI writing can degrade the governance capabilities of economically important organizations, even when such AI is rarely used in equilibrium.
\end{abstract}
...
Delegate-level evidence is consistent with this mechanism and points away from a power-capture
...
average 71,000 ARB (\$92{,}000) in delegated power; low-AI delegates average 1.9 million ARB (\$2.5 million)---a
...
eligible DAOs building on Arbitrum; (iii) 42.02\% to Offchain Labs team members
...
is a central challenge in interpreting the vote-rental market's effects.
...
points---account for 16 of the 306 binary votes (5\%), confirming that most
...
bound $\underline{\pi} > 0$. If $(1 - \underline{\pi}) \cdot I_H < c_{\text{read}}$
...
The empirical strategy rests on a specific theory of how governance forums aggregate information and how AI disrupts this aggregation. This section presents that theory as a unified two-tier model in which readership, posting volume, vote margins, decision quality, and the collapse point are all derived from one set of primitives; complete proofs are in Appendix~\ref{app:theory}.

The model draws on three strands of prior work. \emph{The posting margin} connects to \citet{lohmann1994}, \citet{lohmann1993}, and \citet{austensmith1990}: participation is costly, so equilibrium volume reflects the state of the debate; here the reward to participation is the \emph{audience} itself, which resolves the pivotality problem that afflicts costly participation in a continuum. \emph{The reading threshold} follows \citet{grossman1980} and \citet{dewatripont1999}: information is acquired only when its expected return exceeds its cost; our contribution is that the reader's threshold is governed by the contamination prior and is derived, not assumed. \emph{The voting tier} uses the sincere (decision-theoretic) voting device standard in large-election models \citep{feddersen1996,feddersen1997}, with information aggregation across a continuum of readers in the spirit of Feddersen--Pesendorfer. \emph{The AI shock} is the \citet{akerlof1970} mechanism applied to the demand side of a forum: adverse selection in the pool of messages can unravel the value of reading before contamination is realized.

%% ── 5.1 Setup ────────────────────────────────────────────────────────────────
\subsection{Setup}
\label{sec:setup}

A DAO evaluates proposal $i$ with unknown quality $\theta_i \in \{G, B\}$ and prior $p \equiv \Pr(\theta_i = G) \in (0,1)$; write $x \equiv p(1-p)$ for contestedness and $d \equiv |p - \tfrac12|$ for consensus. There are two tiers of agents.

\emph{Contributors} (a continuum) hold conditionally i.i.d.\ private signals $s_j \in \{h, l\}$ with precision $\Pr(s_j = h \mid G) = \Pr(s_j = l \mid B) = q > \tfrac12$. Contributor $j$ may post her signal at private cost $c_j$, drawn from a continuous distribution $F$. In addition, a mass $z \ge 0$ of AI-generated posts may be present; an AI post is drawn uninformatively from $\{h,l\}$ and is observationally indistinguishable from a human post. If human posting mass is $n$, the AI fraction of the thread is $\pi = z/(z+n)$.

\emph{Voters} (a continuum of mass one; in the application, delegates weighted by voting power) have private tastes $b_j \sim \mathrm{Unif}[-a, a]$, orthogonal to $\theta$---idiosyncratic value from the proposal passing: exposure, vested interest, ideology. Common values are $v(G) = +1$, $v(B) = -1$. Before voting, a voter may pay reading cost $c_r > 0$ to read one post sampled at random from the thread. The proposal passes if the For-share exceeds $\tfrac12$.

Four assumptions close the model; we state them plainly because they are where the modeling work happens. \emph{(A1, exact law of large numbers)}: conditional on $\theta$, cross-sectional averages equal conditional means. \emph{(A2, decision-theoretic voting)}: each voter votes---and values pre-vote information---as if her vote determined the outcome for her own payoff: For iff $\mathbb{E}[v(\theta) \mid \text{information}] + b_j \ge 0$. This is the standard sincere-voting device for large elections \citep[e.g.,][]{feddersen1996}; it \emph{bypasses} rather than solves voter-side pivotality, and it is what disciplines everything that follows---fully instrumental voters would make reading and voting worthless for every atomless agent and could generate none of the observed cross-sectional structure. \emph{(A3, truthful posting)}: a post reveals the contributor's signal; justifiable by reputational cheap-talk arguments since $\theta$ is eventually public. \emph{(A4, attention rewards)}: a contributor who posts receives $R \cdot A$, where $A$ is the mass of readers and $R > 0$ converts audience into utility. Attention is a positive \emph{mass} even though each poster is measure zero, so costly posting survives the continuum without pivotality. This motive is institutionally grounded in our setting: Arbitrum's Delegate Incentive Program compensates visible participation directly, and posting attracts delegated voting power from readers (Section~\ref{sec:mechanism}). What A4 deliberately does \emph{not} assume is that posting signals ability: Appendix~\ref{app:theory} proves that a career-concerns motive---posting to build a reputation for being right---concentrates posting on foregone conclusions for \emph{any} increasing reputation payoff (vindication is most likely when everyone already agrees) and would therefore generate the \emph{opposite} of the posts--margin pattern in the data.

%% ── 5.2 Equilibrium Without AI ───────────────────────────────────────────────
\subsection{Equilibrium}
\label{sec:equilibrium}

Fix contamination $\pi$ for now (the next subsection makes it move; Section~\ref{sec:tippingmodel} makes it an equilibrium object). A sampled post is informative with probability $1-\pi$, so its effective precision is $Q - \tfrac12 = (1-\pi)(q - \tfrac12)$: contamination dilutes the signal linearly. Equilibrium unwinds in three steps, each with a closed form in Appendix~\ref{app:theory}.

\emph{Reading.} The value of reading one post is a tent over tastes: it is zero for voters whose tastes are decided regardless of the message, and peaks at the ex-ante indifferent voter, whose gross value of information is exactly $2p(1-p)(2Q-1)$. Voters read iff this value exceeds $c_r$, so the reading audience $A(p;\pi)$ is single-peaked in contestedness and---for the peak reader---the net value of reading is \emph{exactly}
\begin{equation}
  V(\pi) = (1 - \pi)\,I_H - c_r, \qquad I_H = 2p(1-p)(2q-1),
  \label{eq:readval}
\end{equation}
with reading threshold
\begin{equation}
  \pi \le \pi^* \equiv 1 - \frac{c_r}{I_H}.
  \label{eq:pistar}
\end{equation}
Equations \eqref{eq:readval}--\eqref{eq:pistar} were the primitives of earlier drafts of this model; here they are theorems, with the informational gain $I_H$ pinned to primitives rather than free. Because $I_H$ depends on $p$, the threshold is proposal-specific: consensus proposals stop being read first, and the forum dies from the tails inward.

\emph{Posting.} By A4, contributor $j$ posts iff $c_j \le R\,A(p;\pi)$, so human posting mass is $n(p) = F(R\,A(p))$---posting chases the audience, and the audience sits where information is valuable. To first order, $n(p) \propto p(1-p)$: volume peaks on contested proposals, with the exact closed form available for structural work.

\emph{Voting.} Non-readers vote on prior and taste; readers vote on posterior and taste. By A1, the For-share is deterministic conditional on $\theta$, and the expected margin $\mathbb{E}[M \mid p]$ has a closed form that is \emph{strictly increasing} in consensus $d$ for all parameter values, with floor $|2p-1|/a$ (Appendix~\ref{app:theory}, Theorem~\ref{app:thm:margin}). Margins are interior because voters disagree about \emph{values}, not only facts; they are informative about contestedness because the expected margin rises with consensus. The shorthand ``margin $\propto |2p-1|$'' holds exactly outside a contested window and as the leading term inside it.

The paper's central empirical prediction is then a theorem rather than a calibration:

\begin{proposition}[Signal Content]
\label{prop:signal}
Let proposals draw priors $p_i$ from any nondegenerate distribution. In equilibrium, forum post volume is negatively correlated with the ex-post margin of victory: $\mathrm{Cov}(\text{Posts}_i, \,\text{Margin}_i) < 0$.
\end{proposition}

\begin{proof}[Sketch]
Volume $n(p_i)$ is a nonincreasing function of consensus $d_i$ (posting follows the audience, which follows the value of information); the expected margin is a strictly increasing function of $d_i$ (Theorem~\ref{app:thm:margin}). Two monotone transforms of a single latent variable, moving in opposite directions, covary negatively. Full proof: Appendix~\ref{app:theory}, Theorem~\ref{app:thm:cov}.
\end{proof}
...
\item $\pi \in [0,1]$: a reader's \emph{expectation} that the next post read is AI-generated---a forward-looking object. In the long run $\pi$ converges to $\lambda$, but at a capability shock $\pi$ can jump ahead of $\lambda$: a reader who knows a capable model now exists expects contamination to rise before it visibly has.
\end{itemize}
These two objects drive two distinct mechanisms with opposite timing predictions---which we exploit in Section~\ref{sec:results}.

\paragraph{Volume mechanism (via $\lambda$).} Realized contamination dilutes the sampled-post precision, $Q - \tfrac12 = (1-\lambda)(q-\tfrac12)$, and the equilibrium responds on every margin (Appendix~\ref{app:theory}, Proposition~\ref{app:prop:shock}): the reading audience falls, reaching \emph{exactly zero} at an interior contamination level $\pi^*(p) < 1$; posting falls with its audience---AI does not need to out-argue humans, it only needs to thin the readership that pays for human effort; and margins compress toward the no-forum floor $|2p-1|/a$. This mechanism predicts degradation contemporaneous with, and proportional to, realized AI volume.

\paragraph{Unverifiability mechanism (via $\pi$).} The reading rule \eqref{eq:readval}--\eqref{eq:pistar} operates on the \emph{expectation} $\pi$, not the realization.

\begin{proposition}[Unverifiability Threshold]
\label{prop:threshold}
Each reader's problem is binary, with a hard threshold: she reads iff $\pi \le \pi^*(p) = 1 - c_r/I_H(p)$. Aggregate readership declines in $\pi$ and reaches exactly zero at the interior level $\pi^*(p) < 1$, at which point the posterior equals the prior and the forum is uninformative---the entire informational value of the forum is eliminated at finite contamination, even though a fraction $1 - \pi^*$ of posts remain genuine.
\end{proposition}

The key implication is that $\pi$ can reach $\pi^*$ well before $\lambda$ does: expectations cross the threshold as soon as a sufficiently capable model is publicly available, even while realized AI posts are few. This is the \citet{akerlof1970} logic applied to information: adverse selection can unravel a signaling equilibrium when the \emph{expected} fraction of lemons exceeds a threshold, even when most goods are genuine.

%% ── 5.4 Welfare ─────────────────────────────────────────────────────────────
\subsection{Welfare}
\label{sec:welfare}

What does the DAO lose when the forum dies? Without a forum, the vote follows the prior action and is correct with probability $\max(p, 1-p)$. In equilibrium with a functioning forum, there is an interior \emph{flip window} of contested priors---a symmetric interval around $p = \tfrac12$ whose width increases in signal precision and decreases in reading costs---inside which the aggregated votes of informed readers flip the outcome to match $\theta$ with probability one (the Feddersen--Pesendorfer aggregation property, delivered by A1 across readers). The forum's gross decision value is therefore
\begin{equation}
  V(p) \;=\; \min(p,\,1-p) \cdot \mathbf{1}\{p \in \Omega\},
  \label{eq:welfareV}
\end{equation}
where $\Omega$ is the flip window (Appendix~\ref{app:theory}, Proposition~\ref{app:prop:welfare}): \emph{closeness enters as the level, informativeness as the width}. Contamination destroys value by narrowing the window---consensus proposals fall out first---until it vanishes entirely at $\pi^*(\tfrac12)$, at which point $\min(p,1-p)$ of decision quality is lost on every proposal, maximal ($=\tfrac12$) on the most contested ones. AI destroys the most welfare precisely where good governance matters most.\footnote{Earlier drafts summarized the welfare loss with the smooth kernel $4p(1-p)(q-\tfrac12)^2$. That expression has the right single-peakedness and precision scaling but is not the value of the forum in the derived model; equation \eqref{eq:welfareV} replaces it. None of the calibration in Section~\ref{sec:calibration} used the old kernel, so nothing quantitative changes.}

Comparative statics of the threshold follow directly from \eqref{eq:pistar}: higher reading costs lower $\pi^*$ (a more fragile forum); higher signal informativeness raises it (more robust); and the arrival of an indistinguishable model at $T$ determines \emph{when} expectations cross the threshold, not the threshold itself. Capability below the indistinguishability point has no effect; crossing it causes collapse; further capability gains have no additional marginal effect on an already-collapsed signal. This non-monotone structure motivates using model release dates as breakpoints, and is what the timing test in Section~\ref{sec:results} evaluates.

%% ── 5.5 Tipping ─────────────────────────────────────────────────────────────
\subsection{Endogenous Contamination: A Tipping Theorem}
\label{sec:tippingmodel}

The mechanisms above treat contamination as given. Closing the loop---contamination depends on human posting, which depends on the audience, which depends on contamination---delivers the paper's headline mechanism as a theorem. Fix a representative contested proposal and AI post mass $z$; an equilibrium is a fixed point of the audience map $A \mapsto \mathcal{A}\!\left(z/(z + F(RA))\right)$.

\begin{proposition}[Discontinuous, hysteretic collapse]
\label{prop:tipping}
For any reading cost $c_r \in (0, I_H)$ and any continuous, strictly increasing posting-cost distribution $F$: (i) the collapsed forum ($A = n = 0$) is an equilibrium for every $z > 0$; (ii) there is a tipping point $z^*$ such that healthy and collapsed equilibria coexist for $z < z^*$ and only collapse survives beyond it; (iii) at $z^*$ the audience \emph{jumps} to zero---never a continuous fade-out---because audiences below a positive dead-zone threshold cannot sustain enough human posting to keep expected reading value positive; (iv) the collapse is \emph{hysteretic}: after tipping, reducing AI volume even nearly to zero does not restore the forum, which requires coordinated re-entry of readers, not merely undoing the shock; and (v) at any healthy equilibrium, realized contamination is \emph{strictly below} the reading threshold $\pi^*$---the forum always tips before contamination reaches the level at which reading value would mechanically hit zero. (Appendix~\ref{app:theory}, Theorem~\ref{app:thm:tipping}.)
\end{proposition}

The positive reading cost that generates the unverifiability threshold is the same primitive that makes collapse discontinuous and irreversible. In a numerical benchmark (contested proposal, $q = 0.75$, thin reading margins), the forum tips at realized contamination near $0.7$ against a reading threshold of $0.92$; thinner margins tip far lower. Matching the \emph{observed} collapse at a realized AI share of only $8\%$ (Section~\ref{sec:results}) still requires the expectations channel---$\pi$ jumping ahead of $\lambda$ at the capability release---exactly as argued above; the theorem's contribution is that collapse at partial contamination is an equilibrium property rather than an artifact of one calibration.
...
\label{ass:nr}
Posts address approximately distinct payoff-relevant aspects of a proposal, so the informational value of successive posts is undiminished and the threshold calculus \eqref{eq:readval} applies post-by-post.
\end{assumption}

Non-redundancy is defensible for governance threads---posts flag distinct risks, precedents, and technical issues---but it is an assumption, not a theorem: with redundant signals, only the first-post floor of 19.4 words is fully derived. Under it, the per-delegate surplus integrates to $\bar{n} \cdot (I_H - c_r) \ge 44.2 \times 19.4 \approx 858$ words per proposal, and with $K$ delegates reading, net social surplus per proposal is at least $858K - 10{,}480$ words---positive for $K \ge 13$. Arbitrum's 62 identified active delegates imply net social benefits of at least $\approx 43{,}000$ words per proposal, about four times the writing effort invested in each thread, entirely from the public-good externality of the forum; since 454 unique community members posted in pre-period threads, $K \ge 62$ is itself conservative. These headline magnitudes carry Assumption~\ref{ass:nr} (and the whole-thread reading protocol) wherever they are quoted; the units, the bound logic, and the 19.4-word floor do not.

At the tipping point, the marginal reader's net value falls to zero and this surplus is destroyed. After the break, human contributors still write $\approx 8{,}858$ words per proposal, but these posts are read far less---Section~\ref{sec:reads} documents a 36\% decline in reads per post---an increasingly unrewarded transfer of human effort.\footnote{The model sends human posting to zero at full collapse; the residual observed volume is consistent with a small intrinsic (non-audience) posting motive, which leaves every result above unchanged.}

\paragraph{Delegate alignment and the direction of the bound.}
The voters in the model value correct outcomes directly. That is a strong assumption here: a simple majority requires only about four delegates, so almost every delegate is non-pivotal and the private return to a correct vote is near zero (rational ignorance); the delegation market rewards \emph{observable activity}---posts, rationales, participation---rather than decision quality (this is precisely the attention motive A4); and professional delegates, grant recipients, and vote renters face conflicts. Misalignment does not weaken the calibration---it sharpens its interpretation in two ways. First, misalignment \emph{predicts} the thin private reading margin rather than merely rationalizing it: for a non-pivotal delegate in an activity-rewarded market, reading costs should absorb nearly all of a post's informational value ($c_r/I_H \le 0.92$). Second, reading is itself a public good---an informed vote benefits every token holder---so the private $I_H$ that governs the reading decision is smaller than the social value of the same post. The collapse therefore reveals the \emph{private} surplus margin, and the social value destroyed \emph{exceeds} the figures above: ``at least 858 words'' becomes more conservative once delegates are misaligned, not less. A corollary is that the socially optimal reading threshold lies above the private $\pi^*$---delegates disengage too early from society's standpoint even before AI, and the AI shock only makes an already-inefficient margin bind.

\paragraph{Model and data: two honest discrepancies.} First, the model predicts margin \emph{compression} after the shock (margins fall to the no-forum floor), while post-break margins in the data \emph{rose}. The natural reconciliation is compositional---fewer contested proposals reach a vote once the forum cannot resolve disagreement---which is outside the model; we flag it rather than claim it. Second, as noted above, residual human posting after collapse requires a small intrinsic posting motive. Neither discrepancy affects the pre-break equilibrium predictions that the empirical work tests.
...
near zero from Claude~3 onward. The three significant breaks (GPT-4 Turbo, Claude~3,
GPT-4o) show the same direction---a negative pre-period correlation collapsing to
near zero after the break---while the pre-capability date (Claude~2) shows no break
and the two latest dates are uninformative because the signal had already collapsed
by Claude~3.
...
The timing evidence in Section~\ref{sec:results} weighs against the volume mechanism
...
which $(1 - \underline{\pi}) \cdot I_H < c_{\text{read}}$---which the QLR evidence pins
...
\emph{Token vesting cliff.} Section~\ref{sec:vesting} addresses the March 2024
...
above 20\% hold an average of 71,000 ARB (\$92{,}000) in delegated voting power; those with
AI fraction below 20\% average 1.9 million ARB (\$2.5 million)---a 27$\times$ gap. This gap is not
driven by outliers: the median high-AI delegate holds 40,000 ARB (\$52{,}000), while the median
low-AI delegate holds 680,000 ARB (\$884{,}000)---still a 17$\times$ difference.
...
\begin{figure}[H]
  ...
  \caption{\textbf{Delegate AI Adoption and Voting Power: Cross-Section.}
    ...
    average 71,000 ARB (\$92{,}000) versus 1.9 million ARB (\$2.5 million) for low-AI delegates.}
  \end{singlespace}
  \label{fig:delegates}
\end{figure}
...
The evidence points to unverifiability rather than manipulation. The signal collapses not because
...
tipping point $\pi^*$, and past $\pi^*$ the collapse is discrete, total, and---because a collapsed audience does not re-coordinate merely because the shock recedes---irreversible. A program
...
shocks is among the cleanest available evidence that generative AI degrades a
...
We thank Max Spero for generously providing access to Pangram AI-detection API credits used in this analysis, and Daniel Yue (Georgia Institute of Technology) for continuous insights throughout the project.
...
\section{Model Appendix: Derivations and Proofs}
\label{app:theory}

This appendix derives every result stated in Section~\ref{sec:model} from the model's primitives, in the order: primitives and assumptions; the reading stage; the posting stage (including the failure of the career-concerns alternative); the voting stage and interior margins; the posts--margin covariance; welfare; comparative statics in contamination; and the tipping theorem. Closed forms are verified against direct numerical integration in the replication file \texttt{docs/check\_model.py}.
...
\ref{app:sec:read}--\ref{app:sec:welfare} treat $\pi$ as a parameter;
Section~\ref{app:sec:tipping} makes it an equilibrium object.
...
(Remark~\ref{app:rem:kpost} discusses multi-post reading).
...
\subsubsection{Assumptions, stated honestly}

\begin{assumption}[Exact LLN]\label{app:ass:lln}
...
\begin{assumption}[Decision-theoretic voting and reading]\label{app:ass:sincere}
...
\begin{assumption}[Truthful posting]\label{app:ass:truth}
...
\begin{assumption}[Attention rewards]\label{app:ass:attention}
...
power from readers; direct attention utility. What Assumption~\ref{app:ass:attention}
deliberately does \emph{not} assume is that posting signals ability ---
Theorem~\ref{app:thm:career} shows that motive predicts the wrong sign.
Remark~\ref{app:rem:congestion} shows the results are robust to rewarding
\emph{per-post} rather than per-thread attention.

\begin{assumption}[Interior tastes]\label{app:ass:interior}
$a\ge 1$, so every posterior-based voting threshold lies inside $[-a,a]$ and
vote shares are interior.
\end{assumption}

\begin{assumption}[Pre-AI viability]\label{app:ass:viable}
...
\begin{lemma}[Bayesian identities]\label{app:lem:identities}
\begin{align}
...
\begin{lemma}[Value of reading is a tent]\label{app:lem:tent}
...
\begin{equation}\label{app:eq:peak}
h(p,Q)\;=\;2p(1-p)(2Q-1).
\end{equation}
\end{lemma}
...
\begin{proposition}[Readership]\label{app:prop:readership}
...
\begin{equation}\label{app:eq:readrule}
...
Monotonicity in $x$: differentiating \eqref{app:eq:A} with $D=Q(1-Q)+x(2Q-1)^2$,
\[
\frac{\partial A}{\partial x}
=\frac{2(2Q-1)\,Q(1-Q)+c_r(2Q-1)^2}{2a\,D^2}>0 .
\]
For \eqref{app:eq:readrule}: by Lemma~\ref{app:lem:tent} the peak voter's gross value is
$h(p,Q)=2x(2Q-1)$, and by \eqref{app:eq:Qeff}, $2Q-1=(1-\pi)(2q-1)$, so
$h(p,Q)=(1-\pi)h(p,q)=(1-\pi)I_H$.
\end{proof}

Equation \eqref{app:eq:readrule} is the reading rule \eqref{eq:readval}--\eqref{eq:pistar} of the text, derived exactly for the marginal (peak) reader, with the informational gain $I_H$ pinned to primitives: $I_H=2p(1-p)(2q-1)$. Three consequences:
...
\eqref{app:eq:A} declines in $\pi$ and reaches \emph{exactly zero} at the interior
...
the denominator's $\pi$-dependence is second order). We examine this prediction directly in Section~\ref{sec:reads}.
\end{itemize}

\subsection{The posting stage: volume, and why career concerns fail}
\label{app:sec:post}

\begin{proposition}[Posting volume (Assumptions \ref{app:ass:truth}, \ref{app:ass:attention})]
\label{app:prop:volume}
Contributor $i$ posts iff $c_i\le R\,A(p)$, so equilibrium human posting mass is
\begin{equation}\label{app:eq:n}
...
\begin{proof}
Immediate from Assumption~\ref{app:ass:attention} and
Proposition~\ref{app:prop:readership}. The approximation uses $D\to\tfrac14$ as
...
reading, posting, and --- Section~\ref{app:sec:welfare} --- welfare.

\begin{remark}[Congestion variant]\label{app:rem:congestion}
...
Theorem~\ref{app:thm:cov} is unaffected; only the exact locus ($p(1-p)$ vs.\ its
square root) changes --- a distinction the data cannot make anyway
(the pre-break sample cannot distinguish the two loci).
\end{remark}

\subsubsection{The negative result: reputation for ability posts on the wrong proposals}

A natural alternative posting motive is career concerns --- posting to attract
delegated voting power by signaling ability. We formalize it and prove it
fails, robustly.
...
\begin{theorem}[Career concerns are wrong-signed]\label{app:thm:career}
...
of the data --- and cannot rationalize Proposition~\ref{prop:signal} of the text.
\end{theorem}
...
consensus, not contested debate. It follows that the delegate
posting$\,\leftrightarrow\,$voting-power correlation documented in
Section~\ref{sec:mechanism} should be read as evidence of the attention motive,
not of career concerns.
...
$p$-invariant while vindication probability rises with consensus. The endpoint comparison is proved; the global V-shape is verified
numerically.
\end{remark}

\subsection{The voting stage: interior margins, derived}
\label{app:sec:margin}
...
$\theta=B$), so by Assumption~\ref{app:ass:lln} the For-share is deterministic given
$\theta$:
\[
...
\begin{proposition}[Interior margins, closed form]\label{app:prop:margin}
Let $m_\theta\equiv2\Phi(\theta)-1$. In the active region ($h>c_r$),
\begin{align}
a\,D\;m_G&=Q(1-Q)(2p-1)\;+\;(2Q-1)^2x\;-\;c_r(1-p)(2Q-1),\label{app:eq:mG}\\
a\,D\;m_B&=Q(1-Q)(2p-1)\;-\;(2Q-1)^2x\;+\;c_r\,p\,(2Q-1),\label{app:eq:mB}
\end{align}
...
Assumption~\ref{app:ass:interior}. Under $p\mapsto1-p$ the model is symmetric:
$m_B(1-p)=-m_G(p)$.
\end{proposition}
...
$u=1-2\mu_B-c_r/D_2$ and \eqref{app:eq:A}, write
...
Symmetry: substitute $p\to1-p$ in \eqref{app:eq:mB} ($x$, $D$ invariant).
...
\begin{theorem}[Expected margin rises with consensus]\label{app:thm:margin}
...
\begin{equation}\label{app:eq:floor}
...
\eqref{app:eq:floor}: in regimes (i)--(ii) it binds; in (iii),
...
$p$ because the expected margin strictly increases in consensus $d$; and the text's shorthand ``margin $\propto|2p-1|$'' holds \emph{exactly} outside the
contested window (and as the leading term for weak signals, since
$\E[M\mid\tfrac12]=O((2Q-1)^2)$).

\subsection{The posts--margin covariance}
\label{app:sec:prop1}

\begin{theorem}[Posts--margin covariance]\label{app:thm:cov}
...
$\theta$ contributes nothing). By Propositions \ref{app:prop:readership} and
\ref{app:prop:volume}, $n$ is a nonincreasing function $\nu(d)$; by
Theorem~\ref{app:thm:margin}, $\E[M\mid p]$ is a strictly increasing function
...
The negative covariance between posting volume and margin of victory --- the central empirical object of the text --- is thus a consequence of the equilibrium, with the posting
equilibrium sustained without any pivotality (Assumption~\ref{app:ass:attention})
and the margin process derived (Theorem~\ref{app:thm:margin}). Both conditional
moments are monotone transforms of the single latent $d_i$, which is what licenses the practice of reading margins as inverse measures of contestedness throughout the text.

\begin{remark}[The honest ceiling on empirical content]
Theorem~\ref{app:thm:cov} pins a \emph{sign}, not a locus. On the pre-break sample
($n=29$) the structural locus and a linear reduced form are statistically
indistinguishable ($R^2$: $0.114$ vs.\ $0.111$). The model's
discriminating content is cross-equation (the same $q$ appears in
\eqref{app:eq:A}, \eqref{app:eq:mG}--\eqref{app:eq:mB}, and the welfare window below), and
those over-identifying tests are underpowered on this sample. Deriving the model buys internal rigor, not additional empirical verification.
\end{remark}

\subsection{Welfare: the exact value of the forum}
\label{app:sec:welfare}
...
\begin{proposition}[Exact decision value of the forum]\label{app:prop:welfare}
...
(Proposition~\ref{app:prop:margin}), so correctness turns on $m_B<0$, i.e.
...
\paragraph{Relation to smooth kernels.} The kernel
$4p(1-p)(q-\tfrac12)^2$ used in earlier drafts is not the value of the forum in
any version of the model. In the derived model the exact object is
\eqref{app:eq:V}: a tent $\min(p,1-p)$ truncated to an interior window ---
closeness enters as the level, informativeness as the width --- and contamination
destroys value by narrowing the window until it vanishes. For a \emph{single}
common signal of precision $q$ (closest to a representative-reader framing), the
exact value is the Blackwell tent $(q-\max(p,1-p))_+$, and the belief-accuracy
value is $16[p(1-p)]^2(q-\tfrac12)^2$ to leading order --- note the
\emph{square} on $p(1-p)$. The old monomial matches neither; it is a smooth
mnemonic with the right single-peakedness and precision scaling but no
decision-theoretic pedigree, and the words-based calibration of the text never
used it, so replacing itcosts nothing quantitative.
...
window. The knife-edge flavor of ``probability one'' is Assumption~\ref{app:ass:lln},
not deep structure; what is robust is that decision quality is high inside the
window and prior-driven outside.
\end{remark}

\subsection{The AI shock: dilution, margin compression, and the reading collapse}
\label{app:sec:shock}

AI enters through two channels, both now derived: the sampled-post precision
falls (equation \eqref{app:eq:Qeff}), and the reading calculus \eqref{app:eq:readrule}
deteriorates.

\begin{proposition}[Comparative statics in contamination]\label{app:prop:shock}
...
\begin{proof}
(a) is Proposition~\ref{app:prop:readership} with \eqref{app:eq:Qeff}; (b) immediate.
(c) The floor is attained: $\E[M\mid p]\ge|2p-1|/a$ always
(Theorem~\ref{app:thm:margin}), with equality everywhere once $h\le c_r$, which
...
Part (a) is the precise form Proposition~\ref{prop:threshold} of the text takes in the derived model; in detail:
\begin{itemize}[itemsep=1pt]
\item \emph{At the individual level} the reading decision is discrete:
each reader's problem is binary, and the marginal reader's rule is
\eqref{app:eq:readrule} with a hard threshold.
...
contamination, but a kinked continuous decline rather than a jump. A genuinely \emph{discontinuous} aggregate collapse emerges once contamination is endogenous:
Theorem~\ref{app:thm:tipping}.
\item The text's distinction between expected contamination $\pi$ (the object in the reading rule) and realized share $\lambda$ carries over: the decision-relevant $\pi$ is the reader's forward-looking expectation about the post she is about to read, which jumps at a capability release ahead of $\lambda$, and remains the mechanism for collapse at $\lambda\approx8\%$.
\end{itemize}

\subsection{Endogenous contamination: a tipping theorem}
\label{app:sec:tipping}
...
\begin{theorem}[Discontinuous, hysteretic collapse (representative proposal; exogenous $z$; Assumptions \ref{app:ass:lln}--\ref{app:ass:viable})]
\label{app:thm:tipping}
...
the healthy audience is $A_0=\mathcal{A}(0)>0$ (Assumption~\ref{app:ass:viable});
...
Theorem~\ref{app:thm:tipping} requires no S-shaped adoption curve: a positive
...
irreversible; that unification is the content of Proposition~\ref{prop:tipping} in the text. Numerical benchmark ($x=\tfrac14$,
...
($\pi$ jumping ahead of $\lambda$ at the capability release), as the text argues. The theorem's contribution is that collapse at
...
\begin{remark}[Multi-post reading]
\label{app:rem:kpost}
A full sequential-reading extension --- read posts one at a time at cost $c_r$
each, stopping optimally --- would endogenize how many of the $\bar n$ posts
each voter reads and let the data separate non-redundancy
(Assumption~\ref{ass:nr}) from redundancy via the observed within-thread decay
of reads per post. It is the natural next modeling increment if the calibration
is to be upgraded from illustrative to identified; it is not undertaken here.
\end{remark}
