...
At the tipping point, the representative delegate's net reading value falls toward zero, and on the order of $43{,}000$ words of social surplus per proposal is lost. After the break, human contributors still write $\approx 8{,}858$ words per proposal (the post-period average), but these posts are read far less---Section~\ref{sec:reads} documents a 36\% decline in reads per post---an increasingly unrewarded transfer of human effort. The welfare loss in Section~\ref{sec:welfare} is thus not a continuous erosion but a sharp drop: a realized AI share as low as $8\%$ marks the point at which the forum's value as a public good collapses.
